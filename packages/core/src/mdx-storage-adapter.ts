...
export class MdxStorageAdapter {
  ...
  appendChange(change: Uint8Array): void {
    ...
    this.debounceTimer = setTimeout(() => {
      this.flushChanges().catch((error) => {
        console.error("MdxStorageAdapter: failed to flush changes", error);
      });
    }, DEBOUNCE_MS);
  }
  ...
  shouldCompact(): boolean {
    ...
    if (totalSize > COMPACT_SIZE_THRESHOLD) return true;
    if (this.sequenceNumber >= COMPACT_CHANGES_THRESHOLD) return true;
    if (this.lastCompactTimestamp > 0 && Date.now() - this.lastCompactTimestamp > COMPACT_AGE_THRESHOLD_MS) return true;
    return false;
  }
