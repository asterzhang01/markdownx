/**
 * Preview figures plugin — renders Hugo shortcode figures inline
 * Directly adapted from TEE previewFigures.ts
 */
import {
  WidgetType,
  EditorView,
...
function getFigures(view: EditorView): DecorationSet {
  ...
  for (const { from, to } of view.visibleRanges) {
    ...
    while ((match = INLINE_EXPR_REGEX.exec(text))) {
      ...
      if (srcAttrMatch?.groups) {
        ...
        decorations.push(
          Decoration.mark({
            class: "text-gray-400 font-mono text-left text-sm leading-snug inline-block opacity-60",
